...
const roomImages: Record<RoomId, { src: string; alt: string }> = {
...
let x = 1;
let y = 1;
const heading = document.querySelector<HTMLHeadingElement>('#room-name')!;
...
const directions = document.querySelector<HTMLDivElement>('#directions')!;
...
function render(): void {
  ...
  for (const direction of Object.keys(steps) as Direction[]) {
    const button = directions.querySelector<HTMLButtonElement>(`[data-direction="${direction}"]`)!;
    button.hidden = Boolean(room.blocked[direction]);
  }
  document.querySelectorAll<HTMLElement>('[data-room]').forEach(cell => {
    if (cell.dataset.room === room.id) cell.setAttribute('aria-current', 'location');
...
function move(direction: Direction): void {
  ...
  if (focused instanceof HTMLButtonElement && focused.hidden) {
    directions.querySelector<HTMLButtonElement>('button:not([hidden])')?.focus();
  }
}
